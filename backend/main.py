...
def _load_data():
    ...
    if "sentiment_label" in store.mismatches.columns:
        sent_df = store.mismatches[["review_id", "sentiment_label", "sentiment_score", "is_mismatch"]].drop_duplicates("review_id")
...
def dashboard_summary():
    ...
    mismatch_count = int(store.mismatches.get("is_mismatch", pd.Series(dtype=bool)).sum())
...
def dashboard_trends():
    ...
    merged["date"] = pd.to_datetime(merged["review_date"], errors="coerce").dt.strftime("%Y-%m-%d")
    merged = merged.dropna(subset=["date"])

    grouped = merged.groupby("date")["sentiment_label"].value_counts().unstack(fill_value=0)
    trends = []
...
def reviews(
    ...
    for _, row in page_df.iterrows():
        items.append(ReviewItem(
            ...
            review_text=str(row.get("review_text", "")),
            star_rating=float(row.get("star_rating", 0)),
            review_date=str(row["review_date"]) if pd.notna(row.get("review_date")) else None,
            sentiment_label=str(row["sentiment_label"]) if pd.notna(row.get("sentiment_label")) else None,
...
def aspects():
    ...
    for aspect in sorted(grouped.index):
        ...
        items.append(AspectStat(name=aspect, positive=pos, negative=neg, total=pos + neg))
