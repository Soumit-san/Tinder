...
def predict_batch_task(csv_content: str, text_col: str):
    ...
    rating_col = next((c for c in df.columns if c.lower() in ["star_rating", "rating", "score"]), None)
    app_col = next((c for c in df.columns if c.lower() in ["app_name", "app", "source"]), None)
    date_col = next((c for c in df.columns if c.lower() in ["review_date", "date", "created_at", "at"]), None)
    ...
    if date_col:
        df["review_date"] = pd.to_datetime(df[date_col], errors="coerce").dt.strftime("%Y-%m-%d")
    else:
    ...
    df["is_mismatch"] = ((df["star_rating"] >= 4) & (df["sentiment_label"] == "Negative")) | \
